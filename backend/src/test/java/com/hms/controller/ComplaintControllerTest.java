...
class ComplaintControllerTest {
    ...
    @Mock private ComplaintService service;
    ...
    private Complaint make(Long id) {
        return new Complaint(id, 1L, 1L, ComplaintType.FUSED_LIGHT, "Light broken", ComplaintStatus.PENDING, null);
    }

    @Test @DisplayName("POST /api/complaints: creates complaint")
    ...
    @Test @DisplayName("GET /api/complaints: returns all")
    ...
    @Test @DisplayName("GET /api/complaints/{id}: found")
    void getById() throws Exception {
        when(service.getById(1L)).thenReturn(make(1L));
        mockMvc.perform(get("/api/complaints/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.description").value("Light broken"));
    }

    @Test @DisplayName("GET /api/complaints/{id}: not found returns 404")
    ...
    @Test @DisplayName("PUT /api/complaints/{id}: updates complaint")
    void update() throws Exception {
        when(service.update(eq(1L), any())).thenReturn(make(1L));
        mockMvc.perform(put("/api/complaints/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(make(1L))))
                .andExpect(status().isOk());
    }

    @Test @DisplayName("DELETE /api/complaints/{id}: deletes")
    void deleteComplaint() throws Exception {
        doNothing().when(service).delete(1L);
        mockMvc.perform(delete("/api/complaints/1"))
                .andExpect(status().isOk())
                .andExpect(content().string("Complaint deleted"));
    }

    @Test @DisplayName("GET /api/complaints/hall/{hallId}: by hall")
    void getByHallId() throws Exception {
        when(service.getByHallId(1L)).thenReturn(List.of(make(1L)));
        mockMvc.perform(get("/api/complaints/hall/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(1));
    }

    @Test @DisplayName("GET /api/complaints/student/{studentId}: by student")
    void getByStudentId() throws Exception {
        when(service.getByStudentId(1L)).thenReturn(List.of(make(1L)));
        mockMvc.perform(get("/api/complaints/student/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(1));
    }
}
