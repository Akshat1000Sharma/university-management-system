            ...
            {halls.map((h, i) => {
              ...
              return (
                <Tr key={h.id}>
                  ...
                  <Td>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${h.isNew ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>
                      {h.isNew ? "New" : "Old"}
                    </span>
                  </Td>
                  ...
                  <Td className="text-slate-500 text-sm">{warden?.phone ?? "—"}</Td>
                </Tr>
              );
            })}
          </Table>
